...
import { BetiPredictSimple, MockToken } from "../typechain-types";
...
async function main() {
  ...
  const collateralToken = await MockToken.deploy("BetiPredict Collateral", "BPC", 18) as MockToken;
  ...
  const betiPredict = await BetiPredictSimple.deploy(collateralToken.target) as BetiPredictSimple;
  ...
  // Parse event logs with proper type guards for ethers v6
  const marketCreatedLog = receipt.logs.find(log => 
    log && typeof log === 'object' && 'fragment' in log && 
    log.fragment && log.fragment.name === "MarketCreated"
  );
  
  if (!marketCreatedLog || !('args' in marketCreatedLog)) {
    console.error("Could not find MarketCreated event");
    return;
  }
  
  const marketId = marketCreatedLog.args[0];
