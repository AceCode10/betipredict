...
describe("BetiPredictMarket", function () {
  async function deployFixture() {
    ...
    // Mint USDC for all users (100,000 each)
  ...
  const LIQUIDITY = 10_000n * 10n ** 6n; // 10,000 USDC
  ...
  describe("Market Creation", function () {
    it("Should create a market with valid parameters", async function () {
      ...
      await expect(tx).to.emit(market, "MarketCreated").withArgs(1, "Will it rain?", resolve, owner.address, LIQUIDITY);
  ...
  describe("Trading - Buy", function () {
    it("Should buy YES shares and move price up", async function () {
      ...
      const buyAmount = 1000n * 10n ** 6n; // 1000 USDC
    ...
    it("Should buy NO shares and move price up for NO", async function () {
      const { market, user1 } = await loadFixture(deployFixture);
      await market.createMarket("Test?", futureTime(), LIQUIDITY, ethers.ZeroAddress);
    ...
    it("Should deduct trading fee", async function () {
      ...
      // User spent exactly buyAmount
      expect(balBefore - balAfter).to.equal(buyAmount);

      // Platform should have collected 2% fee
    ...
    it("Should respect slippage protection", async function () {
      ...
      const absurdMinShares = 10_000n * 10n ** 6n; // way too many
  ...
  describe("Trading - Sell", function () {
    it("Should sell YES shares back for collateral", async function () {
      ...
      // Should have received some collateral back
  ...
  describe("Resolution", function () {
    ...
    it("Authorized resolver should be able to resolve", async function () {
      ...
      await expect(market.connect(resolver).resolveMarket(1, 2)).to.not.be.reverted; // No = 2
    });
    ...
    it("Should fail resolving with invalid outcome", async function () {
      ...
      await expect(market.resolveMarket(1, 0)) // Outcome.None = 0
  ...
  describe("Claims", function () {
    it("YES winners should claim winnings after resolution", async function () {
      ...
      // Payout should equal winning shares (1:1 with collateral)
    ...
    it("NO winners should claim winnings", async function () {
      ...
      // Resolve as NO
      await market.resolveMarket(1, 2);
