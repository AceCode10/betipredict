...
type QuestionType = 'yes-no' | 'multi-option' | 'versus'
...
const QUESTION_TEMPLATES = [
  ...
  { label: 'Versus', template: '', type: 'versus' as QuestionType, hint: 'Two-way matchup. e.g. "Messi vs Ronaldo — who scores more this season?"' },
]
              ...
              {questionType !== 'yes-no' && (
                <p className={`text-xs ${textMuted} mt-1.5`}>
                  {questionType === 'multi-option' ? 'Each option will be independently tradable with its own price.' :
                   questionType === 'versus' ? 'Add the two competitors as options (exactly 2).' :
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., fifa.com, premierleague.com"
                  className={`w-full px-3 py-2.5 ${inputBg} border ${borderColor} rounded-lg ${textColor} text-sm`}
                  maxLength={500}
                />
              </div>
